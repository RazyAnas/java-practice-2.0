...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<String> hellos = new ArrayList<>(
                List.of(aText, bText, cText, dText, eText));
        hellos.forEach(s -> System.out.println(s + ": " + s.hashCode()));

        Set<String> mySet = new HashSet<>(hellos);

        System.out.println("mySet = " + mySet);
        System.out.println("# of elements = " + mySet.hashCode());

        for (String setValue : mySet) {
            System.out.print(setValue + ": ");
            for (int i = 0; i < hellos.size(); i++) {
                if (setValue == hellos.get(i)) { // == compares references (memory addresses), not content.
                    System.out.print(i + ", ");
                }
            }
            System.out.println(" ");
...
/*
Java supports four hashed collection implementations:
