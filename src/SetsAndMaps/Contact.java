...
public class Contact {
    ...
    public void addEmail (String companyName) {
        // "Anas Razy" + "Letter Co" → ARazy@letterco.com
        String[] names = name.split(" ");
        String email = "%c%s@%s.com".formatted(name.charAt(0), // First letter of full name → 'A'
                names[names.length - 1],  // Last name → "Razy"
                companyName.replaceAll(" ", "").toLowerCase() // companyName like "Letter Co" → "letterco"
        );
        if (!emails.add(email)) {
